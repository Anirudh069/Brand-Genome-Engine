...
import {
    LayoutDashboard, Settings, CheckCircle, BarChart2, LineChart, Menu, X, Wand2, Wrench
} from 'lucide-react';
import { cn } from "../lib/utils";
import { AnimatePresence } from "framer-motion";

const SidebarContent = ({ activeTab, handleNavClick, navItems, isMobile, setMobileMenuOpen }) => (
    <>
        ...
        <div className="p-6 border-t border-white/5">
            <div className="flex items-center gap-4 p-4 bg-[#111116] rounded-2xl border border-white/5 hover:border-white/10 transition-colors cursor-pointer group">
                ...
                <div className="text-sm flex-1 truncate">
                    ...
                    <p className="text-indigo-400 font-bold text-[10px] tracking-widest uppercase mt-1">Local PoC</p>
                </div>
            </div>
        </div>
    </>
        ...
        <div className="min-h-screen bg-[#050505] flex font-sans text-gray-200 selection:bg-indigo-500/30 selection:text-indigo-200 overflow-hidden relative">
            ...
            {!isMobile && (
                <aside className="w-[280px] shrink-0 bg-[#0A0A0C]/80 backdrop-blur-2xl border-r border-white/5 flex flex-col z-20 shadow-2xl">
                    <SidebarContent activeTab={activeTab} handleNavClick={handleNavClick} navItems={navItems} isMobile={isMobile} setMobileMenuOpen={setMobileMenuOpen} />
                </aside>
            )}
